// Some strange fading going on in this version
...
void colorCoordinates(){
	...
	int y0, y1, y2; // simplify some typing
	...
	if(r0 <= 255 && g0 <= 255 && b0 <= 255 && r0 >= 0 && g0 >= 0 && b0 >= 0 && coordinates_total == (NUM_COORDINATES)){
		...
		y0 = HEIGHT-mousecoordinates[0].y;
		y1 = HEIGHT-mousecoordinates[1].y;
		y2 = HEIGHT-mousecoordinates[2].y;
		denom20 = ((float)(y2-y0));
		denom10 = ((float)(y1-y0));
		denom21 = ((float)(y2-y1));
		...
		for(i = (HEIGHT-mousecoordinates[0].y); i < (HEIGHT-mousecoordinates[1].y); i++){
			if(denom10 != 0 && denom20 != 0){	
				...
				r_l = ((i-y0)/(y1-y0)) * r1 + (1-((float)(i-y0)/(denom10))) * r0;
				r_r = ((i-y0)/(y2-y0)) * r2 + (1-((float)(i-y0)/(denom20))) * r0;
				r = ((x-X_L)/(X_R - X_L)) * r_r + (1-((x-X_L)/(X_R - X_L))) * r_l;
						
				cout << "i = " << i << " rl = " << r_l << " rr = " << r_r << " r = " << r << endl;

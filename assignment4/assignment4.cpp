...
const int maxObj = 40;             // max number in each array
...
sphere sphere_array[maxObj];            // array of spheres
box box_array[maxObj];                  // array of boxes 
bool adjustColorRange = false;  
...
void readSphereFromFile(ifstream &fin, int sphere_index){
     ...
     float spec_exp;
     fin >> x >> y >> z >> r;
     fin >> amb_r >> amb_g >> amb_b;
     fin >> diff_r >> diff_g >> diff_b;
     fin >> spec_r >> spec_g >> spec_b;
     fin >> spec_exp;
     ...
     sphere_array[sphere_index].amb_r = amb_r;
     sphere_array[sphere_index].amb_g = amb_g;
     sphere_array[sphere_index].amb_b = amb_b;
     sphere_array[sphere_index].diff_r = diff_r;
     sphere_array[sphere_index].diff_g = diff_g;
     sphere_array[sphere_index].diff_b = diff_b;
     sphere_array[sphere_index].spec_r = spec_r;
     sphere_array[sphere_index].spec_g = spec_g;
     sphere_array[sphere_index].spec_b = spec_b;
     sphere_array[sphere_index].spec_exp = spec_exp; 
...
void readCubeFromFile(ifstream &fin, int box_index){     
     ...
     box_array[box_index].s = s;     
     box_array[box_index].amb_r = amb_r;
     box_array[box_index].amb_g = amb_g;
     box_array[box_index].amb_b = amb_b;
     box_array[box_index].diff_r = diff_r;
     box_array[box_index].diff_g = diff_g;
     box_array[box_index].diff_b = diff_b;
     box_array[box_index].spec_r = spec_r;
     box_array[box_index].spec_g = spec_g;
     box_array[box_index].spec_b = spec_b;
     box_array[box_index].spec_exp = spec_exp; 
     gmtl::Point3f center(x,y,z);
...
void readFile(){
     ...
     cin >> fname;
     cout << "\nShould I assume colors 0.0 - 1.0 (y/n)? ";
     cin >> colors;
     if(colors == 'y')
       adjustColorRange = true;
     else
       adjustColorRange = false;
...
float clamp(float value, float largestAllowed){
    ...
    for(int j = 0; j < W_HEIGHT; j++){
       ...
       for(int i = 0; i < W_WIDTH; i++){
          ...
          if(object == 2){
            ...
            // an attempt at a better surface normal calculation
            gmtl::Point3f center = ((box_array[index].boxf.mMin + box_array[index].boxf.mMax)/2.0);
            gmtl::Point3f temp = center;
            float dist = box_array[index].s/2.0;
            
            // depending on the intersection side, set the normal properly
            // some artifacts are still showing up though
            for(int param = 0; param < 3; param++){
                if((intersection[param] > (center[param] + dist - 0.001)) && (intersection[param] < (center[param] + dist + 0.001))){
                    temp[param] = center[param] + dist;
                    N = temp - center;
                    break;
                }
                else if((intersection[param] > (center[param] - dist - 0.001)) && (intersection[param] < (center[param] - dist + 0.001))){
                    temp[param] = center[param] - dist;
                    N = temp - center;
                    break;
                }
